        ...
        while True:
            try:
                ...
                if anti_iris_mode.get():
                    message_id = send_message(";_?*8- n0∆g4y :с", conv_id, is_to_ls, captcha_sid=sid, captcha_code=solved_captcha_code)
                ...
                if DEBUG: print("[CAPTCHA]: Captcha needed. Solving...")
                sid = captcha.sid
                if DEBUG: print(f"[CAPTCHA]: Captcha data {sid}")
                solved_captcha_code = vc.solve(sid=sid, s=1)
                if DEBUG: print(f"[CAPTCHA]: Solved code {solved_captcha_code}")

    except Exception as ex:
        if str(ex) == "[9] Flood control":
            print("[INFO]: Flood control, wait!")
            time.sleep(15)
            print("[INFO]: Trying to restart raid!")
            raid(conv_id, is_to_ls, counter_text)
        else:
            print(f"[INFO]: Unknown error {type(ex)} {ex}")
